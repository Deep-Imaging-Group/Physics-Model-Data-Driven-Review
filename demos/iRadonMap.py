...
class BackProjNet(nn.Module):
    def __init__(self, geo, channel=1, learn=False):
        ...
        if self.learn:
            ...
            self.bias = nn.Parameter(torch.zeros(self.geo['nVoxelX']*self.geo['nVoxelY']))
        else:
...
class iRadonMap(nn.Module):
    def __init__(self, options) -> None:
        ...
        # )
        virdet = 0.012858 * 5.95 / 10.856
        filter = torch.empty(2 * 736 - 1)
        ...
        # self.convf = nn.Identity()
        self.spbp = BackProjNet(geo_virtual, learn=True)
        self.rcnn = rCNN()
    ...
    def forward(self, p, mode):
        ...
        if mode == 'pretrain':
            ...
            convp = nn.functional.conv2d(p, self.filter, padding=(0,736-1))
            x = self.spbp(convp)
            x = x * 0.012268 / 2 * 1.2
            out = self.rcnn(x)
        return out
