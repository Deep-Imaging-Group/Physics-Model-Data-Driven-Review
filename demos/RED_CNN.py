...
from recon.models import RED_CNN
...
class net(pl.LightningModule):
    def __init__(self, args):
        ...
        if self.is_vis_show:
            self.vis = Visualizer(env='RED_CNN')
...
class data_loader(Dataset):
    def __init__(self, root, dose, mode):
        self.x_dir_name = 'input_patch_' + dose if mode == 'train' else 'input_' + dose
    ...
    def __getitem__(self, index):
        ...
        file_y = file_x.replace(self.x_dir_name, 'label_patch') if self.mode == 'train' else file_x.replace(self.x_dir_name, 'label')
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(data_loader(args.data_root_dir, args.dose, 'train'), batch_size=args.batch_size*64, shuffle=True, num_workers=args.cpus)
