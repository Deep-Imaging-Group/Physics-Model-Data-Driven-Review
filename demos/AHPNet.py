...
from recon.models import AHPNet
...
class net(pl.LightningModule):
    def __init__(self, args):
        ...
        self.options = options.cuda()
        if self.is_vis_show:
            self.vis = Visualizer(env='AHPNet')
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(0, layer-1):
            loss = loss + F.mse_loss(out[:,[i],:,:], y) * 0.8
        loss = loss + F.mse_loss(out[:,[-1],:,:], y)
